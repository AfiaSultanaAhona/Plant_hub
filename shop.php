...
// Retrieve active customer ID from session
$user_id = $_SESSION['user_id'] ?? $_SESSION['customer_id'] ?? $_SESSION['id'] ?? null;
$total_points = 0;

if ($conn && $user_id) {
    // 1. Fetch loyalty points from the customer table
    $pts_query = mysqli_query($conn, "SELECT loyalty_points FROM customer WHERE customer_id = '$user_id' OR id = '$user_id'");
    
    if ($pts_query && $row = mysqli_fetch_assoc($pts_query)) {
        $total_points = (int)($row['loyalty_points'] ?? 0);
    } else {
        // 2. Fallback: Recalculate based on completed orders (10 PTS per ৳500 spent)
        $orders_query = mysqli_query($conn, "SELECT amount, total_amount, price, total FROM orders WHERE customer_id = '$user_id' OR user_id = '$user_id'");
        if ($orders_query) {
            while ($orow = mysqli_fetch_assoc($orders_query)) {
                $amt = (float)($orow['amount'] ?? $orow['total_amount'] ?? $orow['price'] ?? $orow['total'] ?? 0);
                $total_points += (int)floor($amt / 500) * 10;
            }
        }
    }
}
