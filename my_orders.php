...
if ($conn && $user_id) {
    // Try querying orders table for current user
    $query = "SELECT * FROM orders WHERE customer_id = '$user_id' OR user_id = '$user_id' ORDER BY order_id ASC";
    $res = mysqli_query($conn, $query);

    if (!$res || mysqli_num_rows($res) === 0) {
        // Fallback: Query all records if user-specific query yields no results
        $res = mysqli_query($conn, "SELECT * FROM orders ORDER BY order_id ASC");
    }
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $amount = (float)($row_l['amount'] ?? $row_l['total_amount'] ?? $row_l['price'] ?? 0);
            
            // Calculate 10 points for every ৳500 spent
            $points_earned = (int)floor($amount / 500) * 10;
            $total_points += $points_earned;

            $orders[] = [
                'order_id' => $row_l['order_id'] ?? $row_l['id'] ?? null,
                'plant_name' => $row_l['plant_name'] ?? $row_l['item_name'] ?? 'Plant',
                'amount' => $amount,
...
// Fallback dummy data if database table is empty
if (empty($orders)) {
    $dummy_amounts = [300.00, 500.00, 300.00, 500.00, 220.00, 250.00];
    foreach ($dummy_amounts as $amt) {
        $pts = (int)floor($amt / 500) * 10;
        $total_points += $pts;
        $orders[] = [
            'order_id' => null,
            'plant_name' => 'Plant',
...
<head>
    ...
    <style>
        ...
        .nav-links { display: flex; items-center; gap: 20px; align-items: center; }
...
<div class="top-nav">
    ...
    <div class="nav-links">
        ...
        <a href="cart.php" class="nav-link">My Cart 🛒 (0)</a>
        ...
        <table class="history-table">
            ...
            <tbody>
                ...
                foreach ($orders as $item): 
                    // Dynamic order ID display: uses DB order ID if available, otherwise increments counter sequentially
                    $display_id = !empty($item['order_id']) ? '#' . $item['order_id'] : '#' . $counter;
                ?>
                    <tr>
